...
// ── Remote sync (called by socket-handler.js on file:uploaded/deleted/updated) ──
// api.php's uploadFile/deleteFile/updateFile actions each call
// notifySocketServer(), and socket-handler.js routes those events here
// via updateUI('addFile'|'removeFile'|'updateFileRecord', ...). None of
// the three broadcast payloads carry a field wasTriggeredByCurrentUser()
// recognizes (originatorId/originatorContactId/userId/contactId/actorId
// — see socket-handler.js), so the user who performed the action ALSO
// receives their own broadcast back, after saveFileUpload()/deleteFile()/
// renameFile() already applied the same change locally. Each function
// below checks whether the change is already reflected in `files` and
// no-ops if so, so the boomeranged event can never duplicate or
// re-delete an entry.

function addFile(data) {
    if (!data || !data.id) return;
    if (files.some(f => f.id === data.id)) return; // already applied locally (own action)
    files.unshift(data);
    save('files', files);
    if (currentDealFilesId === data.taskId) renderFilesModalList();
...
function removeFile(id) {
    if (!id) return;
    if (!files.some(f => f.id === id)) return; // already applied locally (own action)
...
function updateFileRecord(data) {
    if (!data || !data.id) return;
    const idx = files.findIndex(f => f.id === data.id);
    if (idx === -1 || files[idx].title === data.title) return; // already applied locally (own action)
    files[idx] = data;
    save('files', files);
    renderFilesModalList();
    if (typeof renderDealModalAttachments === 'function') renderDealModalAttachments();
