...
function isManager() {
    ...
    // Check if role name contains "Manager" (case insensitive)
    if ((role.name || '').toLowerCase().includes('manager')) return true;
    
    // Check inheritance chain
    const chain = (typeof getRoleInheritanceChain === 'function') ? getRoleInheritanceChain(roleId) : [roleId];
    for (const rid of chain) {
        const r = roles.find(rr => rr.id === rid);
        if (r && (r.name || '').toLowerCase().includes('manager')) return true;
    }
    
    return false;
}

function getDepartmentEmployeeIds(departmentId) {
    if (!departmentId) return [];
    return contacts
        .filter(c => c.department === departmentId && c.status === 'active')
        .map(c => c.id);
}
...
function openDealModal(id, defaultStage) {
    ...
    // Strip out the read-only viewer's "Move to…" row and restore the
    // Save/Delete buttons in case this modal DOM was last used by
    // openDealViewOnly()'s no-#deal-view-modal fallback.
    const fallbackMoveRow = document.getElementById('deal-view-move-fallback-row');
    if (fallbackMoveRow) fallbackMoveRow.remove();
...
function renderKanban(options = {}) {
    ...
    // ── Determine which employee IDs to filter by ──
    let filterEmployeeIds = [];
    const currentEmployeeId = getCurrentEmployeeId();
    
    if (isSuperuser) {
        // System Admin: See ALL tasks (no filter)
        filterEmployeeIds = null; // null = show all
    } else if (isManagerUser && currentEmployeeId) {
        // Manager: See tasks for ALL employees in their department
        const deptId = getCurrentUserDepartment();
        if (deptId) {
            filterEmployeeIds = getDepartmentEmployeeIds(deptId);
            // Always include the manager themselves
            if (!filterEmployeeIds.includes(currentEmployeeId)) {
                filterEmployeeIds.push(currentEmployeeId);
            }
        } else {
            // Manager with no department: fallback to just themselves
            filterEmployeeIds = [currentEmployeeId];
        }
    } else if (currentEmployeeId) {
        // Regular employee: See ONLY their own tasks
        filterEmployeeIds = [currentEmployeeId];
    } else {
        // No employee ID (shouldn't happen): show all
        filterEmployeeIds = null;
    }

    const getDealContactIds = d => (d.contactIds && d.contactIds.length) ? d.contactIds : (d.contactId ? [d.contactId] : []);
    
    // ── Filter deals based on employee IDs ──
    let visibleDeals;
    if (filterEmployeeIds === null) {
        // System Admin: show all deals
        visibleDeals = deals;
    } else if (filterEmployeeIds && filterEmployeeIds.length > 0) {
        // Filter to deals assigned to any of the filtered employees
        visibleDeals = deals.filter(d => {
            const assignedIds = getDealContactIds(d);
            return assignedIds.some(id => filterEmployeeIds.includes(id));
        });
    } else {
        // No filter (shouldn't happen): show all
        visibleDeals = deals;
    }
    ...
    const myEmployeeId = currentEmployeeId;
    ...
    // ── Display filter context in pipeline summary ──
    let filterChip = '';
    if (isSuperuser) {
        filterChip = `<div class="pipe-chip" style="background:var(--accent-soft, rgba(99,102,241,0.12));">
            <span>👑 System Admin</span>
            <span class="pipe-chip-val">Viewing all tasks</span>
        </div>`;
    } else if (isManagerUser && filterEmployeeIds && filterEmployeeIds.length > 1) {
        const deptName = getDepartmentName(getCurrentUserDepartment());
        filterChip = `<div class="pipe-chip" style="background:var(--accent-soft, rgba(99,102,241,0.12));">
            <span>📊 Manager</span>
            <span class="pipe-chip-val">${deptName || 'Department'} (${filterEmployeeIds.length} employees)</span>
        </div>`;
    } else if (myEmployeeId) {
        const myName = getContactName(myEmployeeId);
        filterChip = `<div class="pipe-chip" style="background:var(--accent-soft, rgba(99,102,241,0.12));">
            <span>👤 My Tasks</span>
            <span class="pipe-chip-val">${myName}</span>
        </div>`;
    }
  ...
  `;
    ...
    board.innerHTML = visibleStages.map(stage => {
        ...
        const visibilityBadge = stageVisibleRoles.length            ? `<span class="restriction-badge" title="Only visible to: ${stageVisibleRoles.map(rid => (typeof getRoleName === 'function' ? getRoleName(rid) : rid)).join(', ')}">🔒 Restricted</span>`
            : '';
        return `
...
// ── UPDATED dealCardHTML with assignee badges ──
function dealCardHTML(d, stage, employee, stageOptions) {
    ...
    // ── Assignee badges with current user highlight ──
    ...
    // ── MOVE TO dropdown (full-width, permission-aware) ──
    // Shown only if the current user can Grab out of the card's current stage.
    // Options limited to stages the user has Drop permission on (mirrors the
    // same checks moveDealStage() enforces, so drag-and-drop and this
    // dropdown never disagree on what's allowed).
    const moveToStages = canGrabHere
        ? (stageOptions || []).filter(s => s.key !== d.stage && (superuser || typeof canDrop !== 'function' || canDrop(s)))
        : [];
    const moveToDropdown = moveToStages.length ? `
      <div class="deal-card-move-row" style="width:100%;">
        <select class="deal-card-move-select" style="width:100%;" title="Move to another stage"
          onclick="event.stopPropagation()"
          onchange="event.stopPropagation(); if(this.value){ moveDealStage('${d.id}', this.value); } this.selectedIndex=0;">
...
function updateDealCard(dealId, newData) {
    ...
    // Socket update: UI refresh only, huwag mag-save pabalik sa API.
    renderKanban({ save: false });
...
// ── Real-time: remote create / delete ──
// Mirrors updateDealCard()'s pattern. appendDealCard() is called by
// updateDealCard() itself when a socket update references a deal id not
// yet in local state (i.e. it was created elsewhere), and can also be
// wired directly from socket-handler.js for an explicit "deal created"
// event. removeDealCard() is the delete counterpart.
function appendDealCard(dealData, stageKey) {
    if (!dealData || !dealData.id) return;

    const idx = deals.findIndex(d => d.id === dealData.id);
    if (idx !== -1) {
        // Already present (e.g. race with another update) — merge instead
...
window.getCurrentUserDepartment = getCurrentUserDepartment;
window.isManager = isManager;
window.getDepartmentEmployeeIds = getDepartmentEmployeeIds;
