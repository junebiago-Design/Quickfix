...
(function() {
    ...
    function initSocket() {
        ...
        // (connect / disconnect / connect_error are wired up once, below,
        // in the SOCKET LIFECYCLE EVENTS block — that version also does the
        // authenticate handshake and transport-fallback retry, so we don't
        // register a second, simpler set of listeners here.)
...
// ──────────────────────────────────────────────────────────────────
//  REALTIME SYNC — full data reload for every event that changes
//  server-side state.
//
//  Root cause fix: the handlers above only patch the ONE object the
//  event told them about (updateDealCard, addContact, etc). If the
//  server-side change touched anything else — permissions, another
//  client's simultaneous edit, related records — the receiving
//  browser's global arrays (deals, contacts, notes, stages, roles,
//  companies, departments, users) never get refreshed and it keeps
//  rendering stale data. That's invisible on localhost (near-zero
//  latency, no real concurrency) but shows up on Hostinger with real
//  multi-user traffic.
//
//  Every event below also triggers a debounced RealtimeSync.reloadData()
//  (see js/modules/realtime-sync.js), which re-fetches the authoritative
//  dataset from api.php and re-renders. The granular updateUI() calls
//  above still run first for instant local feedback; this reload is the
//  belt-and-suspenders correctness pass that catches everything they miss.
// ──────────────────────────────────────────────────────────────────

const DATA_MUTATING_EVENTS = new Set([
    'deal:created',
    'deal:updated',
    'deal:deleted',
    'deal:stage-changed',
...
socket.onAny((eventName) => {
    if (!DATA_MUTATING_EVENTS.has(eventName)) {
        return;
    }
    if (typeof window.RealtimeSync !== 'undefined' && window.RealtimeSync.reloadData) {
        window.RealtimeSync.reloadData();
    } else {
        console.warn('socket-handler: RealtimeSync not loaded — falling back to granular UI patches only. Load js/modules/realtime-sync.js before socket-handler.js.');
    }
});
