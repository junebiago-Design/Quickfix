...
(function() {
    ...
    const SOCKET_OPTIONS = {
        ...
        autoConnect: true,
        forceNew: false,
        // Add extra headers for localhost development
        extraHeaders: {
    ...
    function initSocket() {
        ...
        socket.on('activity:new', (data) => {
            ...
            // An activity log entry gets written for essentially every
            // meaningful change in the app (deals, notes, contacts,
            // departments, etc — see logActivity()/logNoteActivity() calls
            // throughout), so it's the one signal that "something changed,
            // the Kanban board might be stale" — no need to listen to a
            // dozen separate deal:*/contact:*/department:* events for this.
            //
            // Two guards, both already established elsewhere in this file:
            //   - wasTriggeredByCurrentUser() above already returned if this
            //     was our own echoed action — we have that data locally.
            //   - currentPage === 'kanban': only refetch if Kanban is the
            //     page actually on screen right now (same pattern as
            //     activity.js's dashboard auto-refresh checking
            //     page-dashboard.classList.contains('active')). If the user
            //     is elsewhere, do nothing — Kanban gets fresh data next
            //     time it's opened.
            //
            // Deliberately NOT wired to 'task-activity:new' — that belongs
            // to the per-task audit trail (task-activity.js), which reads
            // its own feed directly and doesn't need a Kanban board reload.
