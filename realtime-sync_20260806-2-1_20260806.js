...
//
//  VERIFIED against api.php AND api-client.js: GET api.php?action=load
//  returns { ok: true, db: { <table>: [...rows] } } for each table in
//  TABLES, and api-client.js's applyDb() confirms the exact global
//  variable each table maps to client-side — notably `tasks` (NOT
//  `announcements`) holds the announcements data, and `login_logs`
//  maps to the global `loginLogs`. No more guessing on those two.
...
(function () {
    ...
    // api.php's 'load' action. Matches api-client.js's applyDb() exactly.
    // activity/taskActivity/files/counters aren't included — those
    // already get their new rows pushed directly via their own socket
    // events and don't need a full-table reload here.
    const FIELD_MAP = {
        deals: 'deals',
        contacts: 'contacts',
        notes: 'notes',
        ...
        users: 'users',
        tasks: 'tasks',       // announcements — confirmed global var is `tasks`, not `announcements`
